...
--//FUN STAR FACT: //--
--// This was originally used for the Tails' Lab Store in SATRB, my level pack :P //--
...
addHook("PlayerSpawn", function(player)
	...
	io.openlocal("client/examples/menu.dat", "a+"):close() --you can change "Name" to something else, change it for the line below too if you do
	local file = io.openlocal("client/examples/menu.dat", "r+")
...
addHook("PlayerThink", function(player)
	if (player.cmd.buttons & BT_CUSTOM1) then
		if not (player.menu.show) then
			...
			local file = io.openlocal("client/examples/menu.dat", "w")
			for e = 1,#player.menu.contents
				for i = 1,#player.menu.contents[e]
